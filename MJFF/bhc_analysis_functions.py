...
summary_order = [
    ...
    "_biomarker_count",      
    "_head_injury_count",    
    "_non_motor_assessment_count",
    "_motor_assessment_count",   
    "_image_exam_count",    
    "_mri_image_count",
    "_aliquot_count",
...
summary_count_headers = {
    ...
    "_biomarker_count": "Biomarker records",      
    "_head_injury_count": "Head Injury records",    
    "_non_motor_assessment_count": "Non Motor Assessment records",
    "_motor_assessment_count": "Motor Assessment records", 
    "_image_exam_count": "Image Exam records"
}
        ...
        if case['diagnoses']:
            ...
            if case['mri_exams'] \
               ...
               and case['mri_exams'][0]['mri_images'][0]['mri_analysis_workflows'][0]['mri_results']:
                    filename = case['mri_exams'][0]['mri_images'][0]['mri_analysis_workflows'][0]['mri_results'][0]['file_name'] 
                    fileID = case['mri_exams'][0]['mri_images'][0]['mri_analysis_workflows'][0]['mri_results'][0]['id']     
...
def run_freesurfer(project_id, subject_id, mri_type = "T1-weighted"):
    ...
    if not os.path.exists(local_output):
        cmd = ['/bin/bash', './scripts/run_freesurfer.sh', subject_id, localpath]
        output = utils.run_command(cmd)
        datetime_end = datetime.datetime.now()
        print("%s: FreeSurfer FINISHED (Total time: %s)." % (str(datetime_end), str(datetime_end-datetime_start)))
    else:
        print("%s: FreeSurfer FINISHED: Results were already found for %s subject." % (str(datetime_start), subject_id))
...
def extract_cortical_measures(project_id, subject_id):
    ...
    if not os.path.exists(local_output):
        ...
        cmd = ['/bin/bash', './scripts/extract.sh', subject_id, subject_path, output_file]
        output = utils.run_command(cmd)
